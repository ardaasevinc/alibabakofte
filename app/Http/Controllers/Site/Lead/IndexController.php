...
use Illuminate\Support\Facades\{Cookie, Log, Auth, Cache}; // Cache eklendi
...
class IndexController extends Controller
{
    ...
    private function processLead(string $buttonId, string $targetUrl): RedirectResponse
    {
        ...
        /* ============================================================
         * 1) BOT VE TARAYICI ENGELEME (Crawler Protection)
         * ============================================================ */
        $bots = [
            'bot', 'crawler', 'spider', 'slurp', 'facebookexternalhit', 
            'meta-external-hit', 'googlebot', 'bingbot', 'yandexbot', 
            'applebot', 'lighthouse', 'headless', 'python', 'curl', 'wget'
        ];
        
        // Eğer User Agent bot listesindeyse veya tamamen boşsa kaydetme, sadece yönlendir.
        if (empty($userAgent) || str($userAgent)->lower()->contains($bots)) {
            return redirect()->to($targetUrl);
        }

        /* ============================================================
         * 2) IP TABANLI HIZ LİMİTİ (IP-Based Throttling)
         * ============================================================ */
        // Saniyeler içinde gelen mükerrer kayıtları engellemek için IP bazlı 20 saniyelik kilit.
        $cacheKey = 'lead_protection_' . md5($ip); 
        if (Cache::has($cacheKey)) {
            return redirect()->to($targetUrl);
        }
        // 20 saniye boyunca bu IP'den yeni kayıt kabul etme.
        Cache::put($cacheKey, true, 20);

        /* ============================================================
         * 3) URL ANALİZİ VE TANIMLAYICILAR
         * ============================================================ */
